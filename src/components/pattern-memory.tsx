...
export function PatternMemory({ patterns }: PatternMemoryProps) {
  ...
  const recentlyDied = patterns.filter(p => p.age >= p.lifespan && p.age < p.lifespan + 2);
      ...
      <CardContent className="pt-4 flex-1 overflow-hidden flex flex-col">
                ...
                >
                  <div className="flex items-start justify-between mb-1.5">
                    <div className="flex flex-col">
                      <span className="font-mono text-[10px] font-bold text-foreground">
                        PATTERN #{pattern.id.split('-')[1].substring(0, 6).toUpperCase()}
                      </span>
                    ...
                    <div className={cn(
                      ...
                      isOld ? "bg-amber-500/20 text-amber-500" : "bg-primary/20 text-primary"
                    )}>
                  ...
                  {isOld && (
                    <div className="absolute inset-0 pointer-events-none">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 h-px bg-destructive/50 animate-pulse" 
        ...
        <div className="mt-3 pt-3 border-t border-primary/10 grid grid-cols-3 gap-2">
          <div className="text-center">
            <div className="font-mono text-[8px] text-muted-foreground uppercase">Spawned</div>
            <div className="font-display text-sm font-bold text-primary">{patterns.length}</div>
          </div>
          <div className="text-center">
            <div className="font-mono text-[8px] text-muted-foreground uppercase">Active</div>
            <div className="font-display text-sm font-bold text-chart-2">{activePatterns.length}</div>
          </div>
          <div className="text-center">
            <div className="font-mono text-[8px] text-muted-foreground uppercase">Died</div>
